...
module.exports = {
    ...
    getCategory: async (req, res) => { 
        try {
            const jwtData = req.category;
            console.log("JWT : ", jwtData); // coba liat data nya
            const categoryDetail = await category.findOne({ 
                where: { id: req.params.id }
            });
            if (!categoryDetail) { return response(res, 404, false, 'category Detail not found', categoryDetail) }
            return response(res, 200, true, 'Success', categoryDetail);
        } catch (error) {
            console.log(error);
            if (error.name === 'SequelizeDatabaseError') {
                return response(res, 400, false, error.message, null);
